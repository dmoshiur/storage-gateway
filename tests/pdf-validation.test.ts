...
import { assertUploadMetadata, assertValidatedBlobPdf, inspectPdfSignature } from "@/lib/validation/pdf";
...
describe("PDF validation", () => {
  ...
  it("rejects Blob metadata, size, or ownership mismatches", () => {
    ...
    expect(() => assertValidatedBlobPdf(valid)).not.toThrow();
    expect(() => assertValidatedBlobPdf({ ...valid, actualSize: 79 })).toThrow(ApiError);
    expect(() => assertValidatedBlobPdf({ ...valid, objectFileId: "other" })).toThrow(ApiError);
  });
});
