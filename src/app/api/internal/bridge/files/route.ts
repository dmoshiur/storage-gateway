 ...
 *
 * The FastAPI bridge validates the document structure itself (header, EOF marker,
 * declared size) and streams the bytes straight into R2 with its own private
 * R2 credentials. Only after the object exists and passes a HEAD verification
 * does the bridge call this endpoint so the document becomes visible to the
 * dashboard, retention, Trash, and NGO website listing with one source of truth
 * for metadata. R2 credentials never travel to the client site.
 */
