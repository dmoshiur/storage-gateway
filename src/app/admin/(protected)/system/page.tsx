...
interface Health { status: "healthy" | "degraded"; version: string; checkedAt: string; services: { application: { status: string }; database: { status: string; latencyMs: number; provider?: string; error?: string }; blobStorage: { status: string; latencyMs: number; authMode?: string | null; configured?: boolean; storeId?: string | null; missingConfiguration?: string[]; errorCode?: string | null; errorName?: string | null; error?: string; hint?: string | null }; authentication: { status: string; provider: string }; scheduledCleanup: { status: string; lastRunAt: string | null; lastSummary: Record<string, number> | null; lastError: string | null } } }
  ...
  return <div className="mx-auto max-w-3xl space-y-4">
    ...
    {!loading && !error && data && <>
      <div className="card-pad"><div className="flex items-center justify-between gap-3"><h2 className="panel-title">Services</h2><span className={data.status === "healthy" ? "badge-success" : "badge-danger"}>{data.status === "healthy" ? "All systems operational" : "Degraded"}</span></div><ul className="mt-2 divide-y divide-line">
        ...
        <StatusRow label="Vercel Private Blob" status={data.services.blobStorage.status} detail={`${data.services.blobStorage.latencyMs} ms${data.services.blobStorage.authMode ? ` · ${data.services.blobStorage.authMode}` : ""}${data.services.blobStorage.storeId ? ` · ${data.services.blobStorage.storeId}` : ""}`} />
        <StatusRow label="Authentication" status={data.services.authentication.status} detail={data.services.authentication.provider} />
      </ul>{(data.services.database.error || data.services.blobStorage.error) && <div className="mt-4 space-y-2">{data.services.database.error && <p className="rounded-lg bg-red-500/10 px-3 py-2 font-mono text-xs text-red-600 dark:text-red-400">{data.services.database.error}</p>}{data.services.blobStorage.error && <div className="rounded-lg bg-red-500/10 px-3 py-2 text-red-600 dark:text-red-400"><p className="text-xs font-medium">{data.services.blobStorage.errorCode ?? "BLOB_ERROR"}{data.services.blobStorage.errorName ? ` · ${data.services.blobStorage.errorName}` : ""}</p><p className="mt-1 break-words font-mono text-xs">{data.services.blobStorage.error}</p>{data.services.blobStorage.hint && <p className="mt-1 text-xs">{data.services.blobStorage.hint}</p>}</div>}</div>}</div>
