...
export function FileManager({ trash = false, canManage = true }: { trash?: boolean; canManage?: boolean }) {
  ...
  const summary = useMemo(() => files.length === 1 ? "1 document" : ` documents`, [files.length]);

  return <div className="space-y-6">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold uppercase tracking-wider text-ngo-600">{trash ? "Recovery area" : "Private document library"}</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">{trash ? "Trash" : "Manage documents"}</h1><p className="mt-2 text-sm text-slate-600">{trash ? "Restore documents before their scheduled permanent deletion." : "Search, retain, view, and safely manage private NGO documents."}</p></div>{trash && <span className="inline-flex items-center gap-2 text-sm font-semibold text-amber-800"><Archive className="h-5 w-5" />Files remain recoverable while in Trash</span>}</header>
    ...
    <section className="panel p-4 sm:p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><label className="sr-only" htmlFor={trash ? "trash-search" : "files-search"}>Search documents</label><input id={trash ? "trash-search" : "files-search"} className="field-input pl-9" value={search} onChange={(event) => { setSearch(event.target.value); setLoading(true); }} placeholder="Search filename, title, description, category, or tag" /></div>{!trash && <><div className="flex items-center gap-2"><Filter className="h-4 w-4 text-slate-500" aria-hidden="true" /><label className="sr-only" htmlFor="file-filter">Filter documents</label><select id="file-filter" className="field-input w-auto" value={filter} onChange={(event) => { setFilter(event.target.value as FileFilter); setLoading(true); }}><option value="all">All active</option><option value="auto_delete">Auto-delete enabled</option><option value="never_delete">Never delete</option><option value="expiring_soon">Expiring soon</option><option value="expired">Expired</option></select></div></>}<div><label className="sr-only" htmlFor="file-sort">Sort documents</label><select id="file-sort" className="field-input w-full lg:w-auto" value={sort} onChange={(event) => { setSort(event.target.value as FileSort); setLoading(true); }}><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="largest">Largest first</option><option value="smallest">Smallest first</option><option value="delete_date">Delete date</option></select></div><Button variant="secondary" className="shrink-0" onClick={refresh} disabled={loading}><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button></div>
      {error && <div className="mt-4"><Notice type="error">{error}</Notice></div>}{searchLimited && <div className="mt-4"><Notice type="warning">Search results are limited to the first 1,000 matching storage records. Narrow your terms if you cannot find an older document.</Notice></div>}
      <div className="mt-5">{loading && files.length === 0 ? <PageLoading label="Loading documents" /> : <FileTable files={files} trash={trash} canManage={canManage} loading={loading} onEdit={setEditing} onTrash={setMoving} onRestore={setRestoring} onPermanentDelete={setPermanentlyDeleting} />}</div>
