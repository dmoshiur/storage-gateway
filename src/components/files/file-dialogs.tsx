...
function EditFileDialogContents({ file, onClose, onSaved }: { file: SerializedFile; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <Dialog open onClose={() => !busy && onClose()} title="Edit PDF details" labelledDescription={`Update the searchable details and retention policy for ${file.originalName}.`}>
...
function RestoreDialogContents({ file, onClose, onRestored }: { file: SerializedFile; onClose: () => void; onRestored: () => void }) {
  ...
  async function restore() {
    ...
    catch (caught) { setError(errorText(caught, "The PDF could not be restored.")); setBusy(false); }
  }
  return <Dialog open onClose={() => !busy && onClose()} title="Restore this PDF?" labelledDescription="The PDF will return to active storage. Its private object must still be available to complete this recovery."><div className="rounded-lg bg-slate-50 p-3 text-sm font-semibold text-ink-900">{file.originalName}</div>{file.permanentDeleteAt && <p className="mt-3 text-sm text-slate-600">Scheduled permanent deletion: {formatDate(file.permanentDeleteAt)}</p>}{error && <div className="mt-4"><Notice type="error">{error}</Notice></div>}<div className="mt-5 flex justify-end gap-3"><Button variant="secondary" onClick={onClose} disabled={busy}>Cancel</Button><Button onClick={restore} disabled={busy}>{busy && <LoaderCircle className="h-4 w-4 animate-spin" />}<RotateCcw className="h-4 w-4" />Restore PDF</Button></div></Dialog>;
}
