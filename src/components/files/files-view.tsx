...
export function FilesView({
  ...
  const debouncedSearch = useDebouncedValue(mode === "trash" ? search : `${search} ${category} ${retention}`.trim());
  ...
  }, [status, filter, apiSort, debouncedSearch, cursor]);
  ...
  const filterKey = `${debouncedSearch}|${status}|${filter}|${apiSort}`;
  ...
  const toggleAll = useCallback(() => {
    if (!data) return;
    setSelected((current) => {
      if (current.size === data.files.length) return new Set();
  ...
  const filtersActive = Boolean(search || category || retention);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {trashView ? (
            session?.role === "admin" && (
              <button type="button" className="btn-danger-soft btn-sm" onClick={emptyTrash}>
      ...
      {selected.size > 0 && (
        <div className="sticky top-16 z-30 flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface-raised p-2.5 shadow-pop animate-slide-up dark:shadow-popdark" role="toolbar" aria-label="Bulk actions">
          ...
          {!trashView && canManage && (
            <>
              <button type="button" className="btn-secondary btn-sm" disabled={bulkBusy} onClick={() => bulk("favorite", { isFavorite: true })}>
                <Heart className="h-3.5 w-3.5" /> Favorite
              </button>
              <button type="button" className="btn-secondary btn-sm" disabled={bulkBusy} onClick={() => bulk("trash")}>
                <Trash2 className="h-3.5 w-3.5" /> Move to Trash
              </button>
            </>
          )}
          {trashView && canManage && (
            <button type="button" className="btn-secondary btn-sm" disabled={bulkBusy} onClick={() => bulk("restore")}>
              <ArchiveRestore className="h-3.5 w-3.5" /> Restore
            </button>
          )}
          {session?.role === "admin" && (
            <button type="button" className="btn-danger-soft btn-sm" disabled={bulkBusy} onClick={() => bulk("delete")}>
