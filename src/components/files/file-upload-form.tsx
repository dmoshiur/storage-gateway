...
function directPut(url: string, file: File, headers: Record<string, string>, onProgress: (percent: number) => void): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error("The PDF could not be uploaded to private storage."));
    xhr.onabort = () => reject(new Error("The PDF upload was cancelled."));
    xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error("The PDF storage service rejected the upload."));
    xhr.send(file);
  });
}

async function clientPdfCheck(file: File): Promise<string | null> {
  if (!/\.pdf$/i.test(file.name)) return "Only files with a .pdf extension can be selected.";
  if (file.size <= 0) return "The selected PDF is empty.";
  const bytes = new Uint8Array(await file.slice(0, 16).arrayBuffer());
  const text = new TextDecoder("latin1").decode(bytes);
  if (!/^%PDF-1\.[0-7](?:[\s%]|$)/.test(text)) return "This file does not appear to have a valid PDF signature.";
  return null;
}
...
export function FileUploadForm({ onUploaded }: { onUploaded: () => void }) {
  ...
  const chooseFile = useCallback(async (candidate: File | undefined) => {
    if (!candidate) return;
    setMessage(null);
    const issue = await clientPdfCheck(candidate);
    if (issue) { setFile(null); setMessage({ type: "error", text: issue }); return; }
    if (settings && candidate.size > settings.maxPdfSizeBytes) {
      setFile(null); setMessage({ type: "error", text: `This PDF is larger than the configured ${formatBytes(settings.maxPdfSizeBytes)} limit.` }); return;
    }
    setFile(candidate);
  ...
  async function upload(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const issue = await clientPdfCheck(file);
      if (issue) throw new ClientApiError("INVALID_PDF", issue);
      if (file.size > settings.maxPdfSizeBytes) throw new ClientApiError("FILE_TOO_LARGE", `This PDF is larger than the configured ${formatBytes(settings.maxPdfSizeBytes)} limit.`);
      ...
      setProgress(null);
      setMessage({ type: "error", text: caught instanceof ClientApiError ? caught.message : "The upload failed. No active PDF record was created; please try again." });
    } finally { setBusy(false); }
  }

  return <section className="panel p-5 sm:p-6"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-ngo-50 text-ngo-700"><Upload className="h-5 w-5" /></span><div><h2 className="font-bold text-ink-900">Upload a PDF</h2><p className="mt-1 text-sm leading-5 text-slate-600">Files upload directly to the private R2 bucket through a short-lived, administrator-authorized URL. The gateway verifies the PDF before it becomes active.</p></div></div>
    <form className="mt-5 space-y-5" onSubmit={upload}>
      ...
      <div><label className="field-label" htmlFor="upload-description">Description <span className="font-normal text-slate-500">(optional)</span></label><textarea id="upload-description" className="field-input min-h-20 resize-y" value={description} maxLength={2000} onChange={(event) => setDescription(event.target.value)} placeholder="A short explanation to help staff find this PDF." disabled={busy} /></div>
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-slate-500">The server verifies filename, size, R2 object metadata, PDF header, and EOF marker. Do not upload confidential material unless your NGO’s policy permits it.</p><Button type="submit" disabled={!file || busy || !settings} className="shrink-0">{busy ? <><LoaderCircle className="h-4 w-4 animate-spin" />Uploading…</> : <><CheckCircle2 className="h-4 w-4" />Upload & verify</>}</Button></div>
    </form>
  </section>;
}
