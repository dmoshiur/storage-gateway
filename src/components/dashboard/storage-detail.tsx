...
interface Stats { totalPdfCount: number; activeFileCount: number; trashFileCount: number; totalStorageBytes: number; activeStorageBytes: number; trashStorageBytes: number; availableBytes: number; usagePercent: number; warningLevel: "normal" | "warning" | "critical"; expiringSoonCount: number; }
...
export function StorageDetail() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-semibold uppercase tracking-wider text-ngo-600">Capacity</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">Storage health</h1><p className="mt-2 text-sm text-slate-600">Metadata-based capacity calculations include active PDFs and recoverable Trash objects.</p></header>{error && <Notice type="error">{error}</Notice>}{stats && <><section className="panel p-5 sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-xl font-bold text-ink-900">{formatBytes(stats.totalStorageBytes)} in use</h2><p className="mt-1 text-sm text-slate-600">{formatBytes(stats.availableBytes)} estimated available · {stats.usagePercent}% of the configured limit</p></div><span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-bold ${stats.warningLevel === "critical" ? "bg-red-50 text-red-800" : stats.warningLevel === "warning" ? "bg-amber-50 text-amber-800" : "bg-ngo-50 text-ngo-700"}`}>{stats.warningLevel !== "normal" && <AlertTriangle className="h-4 w-4" />}{stats.warningLevel === "normal" ? "Capacity normal" : `${stats.warningLevel === "critical" ? "Critical" : "Warning"} threshold reached`}</span></div><div className="mt-6 h-4 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-label={`${stats.usagePercent}% storage used`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={stats.usagePercent}><div className={stats.warningLevel === "critical" ? "h-full bg-red-700" : stats.warningLevel === "warning" ? "h-full bg-amber-500" : "h-full bg-ngo-600"} style={{ width: `${Math.max(stats.usagePercent, 1)}%` }} /></div></section><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat icon={FileText} label="All stored PDFs" value={String(stats.totalPdfCount)} /><Stat icon={HardDrive} label="Active documents" value={`${stats.activeFileCount} · ${formatBytes(stats.activeStorageBytes)}`} /><Stat icon={Archive} label="Recoverable Trash" value={`${stats.trashFileCount} · ${formatBytes(stats.trashStorageBytes)}`} /><Stat icon={AlertTriangle} label="Expiring in 30 days" value={String(stats.expiringSoonCount)} /></section></>}
