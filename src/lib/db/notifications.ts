  ...
  try {
    if (input.dedupeKey) {
      ...
      if (existing.rowCount) {
        await query(`UPDATE notifications SET title = $2, message = $3, created_at = now() WHERE id = $1`, [existing.rows[0].id, input.title, input.message]);
        return;
      }
    }
    await query(
    ...
    await query(`DELETE FROM notifications WHERE id IN (SELECT id FROM notifications ORDER BY created_at DESC OFFSET $1)`, [RETENTION_LIMIT]);
  } catch {
...
export async function pruneNotifications(): Promise<number> {
  const result = await query<{ id: string }>(`DELETE FROM notifications WHERE id IN (SELECT id FROM notifications ORDER BY created_at DESC OFFSET $1) RETURNING id`, [RETENTION_LIMIT]);
...
export async function listNotifications(input: { limit?: number; unreadOnly?: boolean } = {}): Promise<{ notifications: SerializedNotification[]; unreadCount: number }> {
  ...
  const [result, unread] = await Promise.all([
    ...
    query<{ count: string }>(`SELECT count(*)::text AS count FROM notifications WHERE read = false`),
  ]);
