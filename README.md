...
Admin browser / trusted API client
      ...
      Next.js Server and API routes
          │              │
          ▼              ▼
     PostgreSQL     Vercel Private Blob
...
- PostgreSQL stores metadata only. Actual document bytes are stored only in a private Vercel Blob store.
...
- Vercel Cron runs the retention and cleanup worker. PostgreSQL advisory/row locks prevent overlapping cleanup runs.
- No demo records or in-memory backend is used by application routes.

## Quick start

Requirements: Node.js 20.9+, PostgreSQL 14+, and a private Vercel Blob store.

```bash
npm install
cp .env.example .env.local
# Configure DATABASE_URL and BLOB_READ_WRITE_TOKEN in .env.local
npm run db:migrate
npm run dev
```
...
| `DIRECT_DATABASE_URL` | migrations | Optional direct PostgreSQL URL for `npm run db:migrate` |
...
1. `POST /api/files/upload/init` validates metadata and creates an `uploading` PostgreSQL row.
2. The browser uploads bytes to a constrained private Blob operation.
3. `POST /api/files/:id/complete` checks Blob metadata and activates the PostgreSQL row.
4. Preview/download routes authenticate the caller, load metadata from PostgreSQL, and stream bytes through Next.js.
5. Trash, restore, and permanent delete update the lifecycle in PostgreSQL and delete Blob bytes only after the state transition is safe.
...
Administrators create scoped bearer keys from `/admin/api` or `POST /api/api-keys`. The raw value is displayed exactly once. PostgreSQL stores only a SHA-256 digest; revocation and expiry are checked on every request. API responses use JSON envelopes with a request ID and stable error code. The API bridge records request and upload activity and writes audit events.

Supported scopes are defined in `src/lib/security/scopes.ts`. Rate limits apply before expensive operations. Cookie-authenticated mutations require same-origin requests; server-to-server API clients use a PostgreSQL-backed API key whose raw secret is shown only once.

## Retention and cleanup

Each file has an explicit retention policy. A daily Vercel Cron request to `/api/cron/cleanup` authenticates with `CRON_SECRET`, acquires the PostgreSQL cleanup lock, processes each file independently, and records failures in `audit_logs`. With Trash enabled, automatic expiry first moves a file to Trash. The next cleanup permanently removes expired Trash metadata and Blob bytes. Stale uploads are removed as well.
...
src/lib/db/             PostgreSQL repositories
